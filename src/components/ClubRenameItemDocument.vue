<template>
  <q-item class="popup-component" clickable>
    <q-img class="popup-png" src="/assets/icons/doc_popup/pen.png" />
    <q-item-section>Переименовать</q-item-section>

    <q-popup-edit v-model="cur_name" :validate="(val) => val.length < 150" v-slot="scope">
      <q-input v-model="scope.value" :model-value="scope.value" hint="Введите имя документа"
        :rules="[(val) => scope.validate(val) || 'Слишком длинное название']">
        <template v-slot:after>
          <q-btn flat dense color="negative" icon="cancel" @click.stop.prevent="scope.cancel" />
          <q-btn flat dense color="positive" icon="check_circle" @click.stop.prevent="filesApi.updateFile(scope.value, props.prop_doc, prop_page_id)
